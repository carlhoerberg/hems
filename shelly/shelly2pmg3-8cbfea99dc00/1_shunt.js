...
const TARGET_SUPPLY_AIR_TEMP = 20.0 // Target supply air temperature (°C)
const MIN_SUPPLY = 2.0 // Minimum allowed supply temperature (°C)
const MAX_SUPPLY = 60.0 // Maximum allowed supply temperature (°C)
const SUPPLY_AIR_Kp = 1.5 // Proportional gain for supply air temperature control
...
function calculateSupplyTemperature (callback) {
  fetchSupplyAirTemperature(function (T_supply_air, error) {
    ...
    // Dead band: if error is within ±1°C, no adjustment needed
    if (Math.abs(airTempError) <= 1.0) {
      print('Air temp error within dead band (±1°C), no adjustment needed')
      callback(SetpointTemperature.getValue())
      return
    }
